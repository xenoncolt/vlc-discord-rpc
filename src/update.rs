use std::{env, fs, io::{stdin, stdout, Write}, path, process::Command, thread::sleep, time::Duration };
use semver::Version;
...
pub async fn update() {
    ...
    let current_exe = env::current_exe().expect("Failed get current exe file path");
    ...
    if latest_version == current_version && path::Path::new(&temp_file).exists() && exe_path == current_exe {
